...
// Check if the student is already logged in, and redirect to the home page
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($reset_code_err) && empty($password_err) && empty($confirm_password_err)) {
        ...
        $result = mysqli_query($conn, "SELECT * FROM student WHERE reset_code = '$resetcode'");
        ...
        if (!$user) {
            ...
            $stmt = mysqli_query($conn, "UPDATE student SET password = '$hashed_password', reset_code = NULL, reset_timestamp = NULL WHERE id = '$user_id'");
            if($stmt){
