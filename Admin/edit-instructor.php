...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["email"]))) {
        ...
        $sql = "SELECT id FROM instructor WHERE email = ? AND id != ?";

        if ($stmt = $conn->prepare($sql)) {
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("si", $param_email, $instructor_id);
    ...
    if (empty($email_err) && empty($password_err) && empty($confirm_password_err) && empty($first_name_err) && empty($last_name_err)) {
        ...
        $sql = "UPDATE instructor SET email = '$email', first_name = '$first_name', last_name = '$last_name', password = '$password' WHERE id = '$instructor_id'";
        $exec = mysqli_query($conn, $sql);
        if ($exec) {
            header("location: ./admin-manage-instructor.php");
            exit();
        } else {
            echo "Oops! Something went wrong. Please try again later.";
        }
