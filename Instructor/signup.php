...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["email"]))) {
        ...
        $sql = "SELECT id FROM instructor WHERE email = ?";
...
if (empty(trim($_POST["dob"]))) {
    ...
    if (!$date || $date->format('Y-m-d') !== $dob) {
        $dob_err = "Please enter a valid date of birth (yyyy-mm-dd).";
    }
}
...
if (empty($email_err) && empty($first_name_err) && empty($last_name_err) && empty($password_err) && empty($confirm_password_err) && empty($gender_err) && empty($speciality_err) && empty($dob_err)) {
    ...
    $sql = "INSERT INTO instructor (email, first_name, last_name, password, gender, speciality, dob) VALUES (?, ?, ?, ?, ?, ?, ?)";

    if ($stmt = $conn->prepare($sql)) {
        // Bind variables to the prepared statement as parameters
        $stmt->bind_param("sssssss", $email, $first_name, $last_name, $password, $gender, $speciality, $dob);
